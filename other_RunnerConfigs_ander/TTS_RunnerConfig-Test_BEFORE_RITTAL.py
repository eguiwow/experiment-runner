...
def get_credentials(host_name):
    # get credentials for either GLs or Rittal system (Rittal does not have a host)
...
class RunnerConfig:
    ...
    def before_run(self) -> None:
        ...
        host, username, password = get_credentials(self.LOGGING_GL)
        ssh_port_forwarding_command = f"echo {password} | ssh -L 8080:192.168.0.200:80 {username}@{host}"
        remote_command(self.LOGGING_GL, ssh_port_forwarding_command, "SSH Forwarding started", background=False)
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        _, userRittal, passRittal = get_credentials("Rittal")
        ...
        stdin, stdout, stderr = con.exec_command(f"cd ~/tts-thesis; echo {password} | sudo -S docker-compose up -d")

        output.console_log(f"Output: {stdout.read()}")

        err = stderr.read()
        # FOR SOME REASON docker-compose sends its output to stderr, making this if useless
        # if err != b'':
        #     output.console_log(err)
    ...
    def interact(self, context: RunnerContext) -> None:
        ...
        if self.SUT is "GL5":
            os.system(f"for i in $(ls ~/THESIS/tts-thesis-master/k6-test-GL5); "
                    f"do k6 run --http-debug - <~/THESIS/tts-thesis-master/k6-test-GL5/$i/script.js --vus {workload} --duration 20s ; done")
        else:
