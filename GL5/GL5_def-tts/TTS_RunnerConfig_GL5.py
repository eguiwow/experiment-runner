...
def get_Rittal_credentials():
    username = os.getenv("RITTAL_U_2")
    password = os.getenv("RITTAL_P_2")
...
class RunnerConfig:
        ...
        if num_containers < 68:
            ...
            if num_containers < 68:
                ...
                if not self.testing:
                    ...
                    self.con_SUT.execute_remote_command(f"echo {passSUT} | sudo reboot", "REBOOT SUT")
    ...
    def interact(self, context: RunnerContext) -> None:
        ...
        if time.time() - timestamp_ini < 120:
            ...
            self.discard_run = True
            with open(f'logfile{self.SUT}.log', 'a') as file:
                timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
                file.write(f"[{context.run_variation['runs']}-{timestamp}] [{workload}] FAILED during K6-test \n")
